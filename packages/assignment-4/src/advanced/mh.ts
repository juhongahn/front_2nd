...
function main() {
  ...
  const addHTMLToParent = (literalHtml, parentNoe) => {
    if (!parentNoe instanceof Node) throw Error('parent is not Node');

    const childFragment = document.createRange().createContextualFragment(literalHtml);
    parentNoe.appendChild(childFragment);
  };
