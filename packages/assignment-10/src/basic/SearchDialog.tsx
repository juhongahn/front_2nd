...
import SearchDialogTableItem from "./SearchDialogTableItem.tsx";
...
const fetchMajors = (() => {
  let majorsCache: Promise<any> | null = null;
...
const fetchLiberalArts = (() => {
  let liberalArtsCache: Promise<any> | null = null;
  return () => {
    if (!liberalArtsCache) {
...
// await를 하면(리스트에 비동기가 없으면) Promise.all이 동기적으로 작동한다.
...
// 필터링 옵션 분리 시도, onChange로 전달되는 changeSearchOption 때문에 리렌더링 발생
...
const SearchCreditsSelect = memo(
  ({
    credits,
    onChange,
  }: {
...
const SearchGradesCheckboxes = memo(
  ({
    grades,
    onChange,
  }: {
...
const SearchDaysCheckboxes = memo(
  ({
    days,
    onChange,
  }: {
...
const SearchTimesCheckboxes = memo(
  ({
    times,
    onChange,
  }: {
...
const SearchMajorsCheckboxes = memo(
  ({
    majors,
    allMajors,
    onChange,
  ...
  const changeSearchOption = useCallback(
    (field: keyof SearchOption, value: SearchOption[typeof field]) => {
      ...
      setSearchOptions((prevOptions) => {
        if (prevOptions[field] === value) return prevOptions;
        return { ...prevOptions, [field]: value };
      });
      ...
      <ModalContent maxW="90vw" w="1000px">
        ...
        <ModalBody>
          <VStack spacing={4} align="stretch">
            ...
            <HStack spacing={4}>
              <SearchGradesCheckboxes
                grades={searchOptions.grades}
                onChange={changeSearchOption}
              />

              <SearchDaysCheckboxes
            ...
            <HStack spacing={4}>
              <SearchTimesCheckboxes
                times={searchOptions.times}
                onChange={changeSearchOption}
              />

              <SearchMajorsCheckboxes
